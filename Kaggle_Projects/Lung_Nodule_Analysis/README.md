...
- **[整合模型並分析](./analysis-of-luna-pytorch.ipynb)** -       整合上述3個模型，建立CT掃描程序，並加以分析。  
...
- **資料來源說明**  
  ...
  若載入記憶體需要**240GB**以上之記憶體，因為Kaggle的GPU上提供**15G**快取記憶體，  
  所以此專案將檔案快取存入硬碟空間以減少記憶體之消耗，  
  又因平台上只提供**19.5G**之儲存空間，故此專案在此需**減少訓練樣本數**，此模型抽取之樣本總數為**30000筆**資料。  
...
- **資料處理**  
  ...
  此模型為**結節的判斷**，所以將資料中有關結節之資訊擷取出，包含結節及非結節資料。  
  因為我們資料來源為一個**3D圖檔**，我們需要知道結節位置，所以須將**座標(xyz)轉換為位置(IRC)**(Index,Raw,Col)資訊。  
  這裡不將整個CT作為Input，而是將結節從資料來源中切下為一個**切塊**(chunk)，以此作為訓練之輸入。  
...
- **建立Dataset**  
  ...
  在此又會發生結節資料重複輸入狀況，所以我們做一個將輸入資料做**隨機處理**(翻轉,縮放,隨機排列等)，避免過度配適情形發生。  
...
- **訓練結果**    
  ...
  此訓練使用**平衡資料**及**資料擴增**函數，並**迭代10次**做模型訓練。結果如下：
  ```
  ...
  **若要加強模型的表現，可以做以下修正**：  
  1.**增加訓練樣本數**，使用更多樣化的樣本作訓練減少過度配適，提高準確率。  
  2.**修改模型**，增加模型參數，及使用Dropout，都可以讓模型減少過度配適現象。  
...
- **建立Dataset**    
  ...
  此處的訓練樣本**將96*96大小的圖檔隨機取出64*64大小**，作為資料擴增的方式之一。  
...
**程式說明**：  
  ...
  此步驟為最後整合程式，將前面所訓練的**影像分割模型、結節分類模型、惡性結節分類模型**整合在一起，
  使們只要透過輸入CT影像檔案，即可得出是否有良性及惡性結節之預測。
  此程式將取出未訓練之樣本資料，將其輸入至整合預測程式中，並做預測結果之分析。
...
- **建立Dataset**  
  ...
  因為這裡有用到所有模型，所以須**匯入前3個模型之Dataset**，  
  不需再對其作修改。  
...
- **建立模型**  
  ...
  有三個模型，需用到三個模型程式，但是兩個分類模型用的架構是同一個，所以這裡匯入模型為：  
  - 結節分類模型  
  - 影像分割模型  
...
- **建立分析程式**
  ...
  初始化模型，將三個模型之**權重資料載入**模型中。  
  建立資料集，取出**所有驗證樣本**資料，確保資料來源未經過訓練。  
  使用所有驗證資料做預測，將所有驗證資料之序號依序輸入模型中。  
  **建立遮罩圖像分割功能**，此程式會將遮罩圖像分組，使其成為多筆結節資料。  
  **建立混淆矩陣函數**，使用混淆矩陣紀錄統計資料，將結果記錄於矩陣中並顯示。  
    
  **資料輸入模型流程**：  
  1.取得CT序號，並將資料打包成**Ct**(class)型態資料。  
  2.使用分割模型資料並取得一筆資料，輸入**結節影像分割模型**中進行預測。  
  3.將分割輸出結果放入**遮罩分割**程式，處理為一筆一筆的結節資料。  
  4.將這些結節資料整理為結節資料集之Dataloader。  
  5.將資料依序放入**結節分類模型**及**惡性結節預測模型**做處理。  
  6.**過濾掉機率小於0.5**的結節預測資料。  
  7.將預測結果輸入**混淆矩陣**中，並印出每筆結果。  
  8.所有資料預測完後，**印出所有統計結果**。  

  **一共輸入52張CT影像資料，總共耗時21分鐘，預測結果如下**：  
  這裡總共做了3個預測，但是結果只有些微差距。所以就拿第一個預測做說明：  
  每筆CT資料處理完畢後會印出結果，最後會印出所有統計結果。  
  ```
  ...
  - **統計結果說明**：  
    ...
    其中**正確預測良性結節為29個**，**正確預測惡性結節為19個**  
      
    **良性結節**預測**準確率**為：29 / 44 = **65.9%**  
    **惡性結節**預測**準確率**為：19 / 28 = **67.8%**  
      
    此次訓練模型預測之**準確率大概為 66.85%**  
    其中惡性結節預測準確率略高一些。  
        
    可以看到惡性結節完全沒預測到有7個，良性結節有9個，代表模型還有很大的改進空間。  
      
- **總結**  
    
  此次訓練我們只取一半的資料做訓練，因為資源有限，包含硬碟空間及GPU記憶體空間，無法將所有資料載入進行訓練，
  若所有資料加入訓練估計分數會更好一點。此次訓練影像分割模型表現較差些，因為要訓練一個深度CNN模型需要較高的硬體需求，
  若能改善影像分割的準確度，模型整體的準確率一定可以大大提升。
